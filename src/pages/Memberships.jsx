...
const STATUS = {
  ...
  Suspended: { color: "#f59e0b", bg: "rgba(245,158,11,0.1)", label: "Suspended" },
};
...
function MembershipCard({ membership, member }) { 
  ...
  const s = STATUS[membership.status] || { color: "#555", bg: "#1e1e1e", label: membership.status };
      ...
      <div style={{ padding: "18px 20px 20px", display: "flex", flexDirection: "column", gap: 14, flex: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            ...
            background: "#252836", border: "1px solid #2e3244",
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{
              ...
              color: "#4a5068", fontWeight: 500, marginTop: 1,
            }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <div>
              <div style={{
                ...
                textTransform: "capitalize", color: "#3a4060", marginBottom: 2,
              }}>Membership #</div>
              <div style={{
                fontFamily: "Barlow Condensed, sans-serif",
                fontWeight: 800, fontSize: "1.1rem",
                color: "#8b92b0", letterSpacing: "0.03em",
              }}>
          ...
          <div style={{
            ...
            <span style={{
              ...
              background: s.color,
              boxShadow: membership.status === "Active" ? `0 0 5px ${s.color}` : "none",
            }} />
            {s.label}
          </div>
        </div>
        ...
        <div style={{
          ...
          gap: "6px 12px",
          background: "#171a26", borderRadius: 6,
          padding: "10px 12px",
        }}>
          {[
...
export default function Memberships() {
  ...
  useEffect(() => {
    async function load() { // Load memberships and members in parallel, then set state accordingly
      try {
  ...
  const filtered = memberships.filter(ms => {
    ...
    const matchSearch = !q || name.includes(q) || String(ms.member_id).includes(q) || String(ms.membership_id).includes(q);
    return matchFilter && matchSearch;
  });

  const counts = FILTERS.reduce((acc, f) => {
    acc[f] = f === "All" ? memberships.length : memberships.filter(m => m.status === f).length;
          ...
          <Box sx={{ display: "flex", alignItems: "baseline", gap: 2, flexWrap: "wrap" }}>
            ...
            {!loading && (
              <Typography sx={{
                ...
                fontSize: "1.4rem", color: "#FF5722", lineHeight: 1,
              }}>
                {filtered.length}
              </Typography>
            )}
          ...
          {!loading && !error && (
            <Box sx={{ display: "flex", gap: 3, mt: 2, flexWrap: "wrap" }}>
              {[
                ...
                { label: "Suspended", count: counts.Suspended, color: "#f59e0b" },
                ...
                <div key={label} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{ fontFamily: "Inter, sans-serif", fontSize: "0.75rem", color: "#4a5068", fontWeight: 500 }}>
                    <span style={{ color: "#8b92b0", fontWeight: 700 }}>{count}</span> {label}
                  </span>
                </div>
              ))}
            </Box>
